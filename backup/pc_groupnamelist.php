...
<body>
	...
	$sql = "SELECT * FROM dynamiclist where route = '$routename'  order by groupname asc,depdate asc";
	...
	echo "<th id='head01' align='center'><a href='pc_account_admin.php?'>管理</th>";
	echo "<th id='head01'><a href='pc_logout.php?'>$indate</th>";
	echo "</thead>";
	echo "<thead>";
	echo "<th id='head02'>同線別子團列表</th>";
	echo "</thead>";
	echo "<thead>";
	echo "<th id='head03' width=5% >出發日期</th>";
	...
	echo "<th id='head03' width=6% >班機</th>";
	echo "<th id='head03' width=5% >出入點</th>";
	echo "<th id='head03' width=17% align='left'>員工備註</th>";
	echo "</thead>";
	echo "</table>";
	echo "</div>";
	...
	while ($row = mysqli_fetch_assoc($result) and $j <= $records_per_page) {
		...
		echo "<tr>";
		echo "<td id='head04' bgcolor='$color' width=5%><font color=$font_color>" . $row['depdate'] . "</font></td>";
		echo "<td id='head04' align='center' bgcolor='$color' width=5%><font color=$font_color>" . $row['groupid'] . "</font></td>";
		echo "<td id='head04' align='left' bgcolor='$color' width=12%><font color=$font_color>" .$lock. $row['groupname'] . "</font></td>";
		...
		echo "<td id='head04' align='center' bgcolor='$color' width=6%><font color=$font_color>" . $row['flight'] . "</font></td>";
		echo "<td id='head04' align='center' bgcolor='$color' width=5%><font color=$font_color>" . $row['inoutpoint'] . "</font></td>";
		echo "<td id='head04' align='left' bgcolor='$color' width=17% ><font color=$font_color>" . $row['groupmemo'] . "</font></td>";
		echo "</tr>";
	...
	echo "<p align='center'>";
	if ($page > 1)
		echo "<a href='pc_pc_groupnamelist.php?page=" . ($page - 1) . "'>上一頁</a> ";

	for ($i = 1; $i <= $total_pages; $i++) {
		if ($i == $page)
			echo "$i ";
		else
			echo "<a href='pc_groupnamelist.php?page=$i'>$i</a> ";
	}

	if ($page < $total_pages)
		echo "<a href='pc_groupnamelist.php?page=" . ($page + 1) . "'>下一頁</a> ";
