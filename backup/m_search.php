...
<body>
	...
	//$sql = "SELECT * FROM dynamiclist where groupmemo like '%$keyword%'  order by depdate ";
	//	$sql = "SELECT * FROM forecase ";
    $sql = "SELECT * FROM dynamiclist where groupmemo like '%$keyword%'  or groupid like '%$keyword%' or groupname like '%$keyword%' or productname like '%$keyword%'  order by depdate ";
	...
	echo "<p align='center'>";
	if ($page > 1)
		echo "<font size='28px'><a href='m_push.php?page=" . ($page - 1) . "'>上一頁</a></font>";

	for ($i = 1; $i <= $total_pages; $i++) {
		if ($i == $page)
			echo "<font size='28px'>".$i."</font> ";
		else
			echo "<font size='28px'><a href='m_push.php?page=$i'>$i</a></font> ";
	}

	if ($page < $total_pages)
		echo "<font size='28px'><a href='m_push.php?page=" . ($page + 1) . "'>下一頁</a></font> ";
