...
<body>
	...
	echo "<th id='head03' width=15% align='left'>員工備註</th>";
	echo "</thead>";
	echo "</table>";
	echo "</div>";
	...
	while ($row = mysqli_fetch_assoc($result) and $j <= $records_per_page) {
		...
		echo "<td id='head04' align='left' bgcolor='$color' width=15% ><font color=$font_color>" . $row['groupmemo'] . "</font></td>";
		echo "</tr>";
	...
	echo "<p align='center'>";
	if ($page > 1)
		echo "<a href='pc_getdeposit.php?page=" . ($page - 1) . "'>上一頁</a> ";

	for ($i = 1; $i <= $total_pages; $i++) {
		if ($i == $page)
			echo "$i ";
		else
			echo "<a href='pc_getdeposit.php?page=$i'>$i</a> ";
	}

	if ($page < $total_pages)
		echo "<a href='pc_getdeposit.php?page=" . ($page + 1) . "'>下一頁</a> ";
