...
<body>
	...
	$sql = "SELECT * FROM dynamiclist where groupname = '$groupname' order by depdate ";
	...
	echo "<th id='head03' width=5% >狀態</th>";
	...
	while ($row = mysqli_fetch_assoc($result) and $j <= $records_per_page) {
		...
		echo "<td id='head04' align='center' bgcolor='$color' width=5%><font color=$font_color>" . $row['status'] . "</font></td>";
	...
	echo "<p align='center'>";
	if ($page > 1)
		echo "<a href='pc_grouplist.php?page=" . ($page - 1) . "'>上一頁</a> ";

	for ($i = 1; $i <= $total_pages; $i++) {
		if ($i == $page)
			echo "$i ";
		else
			echo "<a href='pc_grouplist.php?page=$i'>$i</a> ";
	}

	if ($page < $total_pages)
		echo "<a href='pc_grouplist.php?page=" . ($page + 1) . "'>下一頁</a> ";
