...
<?php
    ...
    if (!$fp = fopen($dbname,"r")){ //開檔判斷
            ...
            //echo $temp;
                if ($row>3){
                    ...
                    */
                    ...
                    $sql="INSERT INTO dynamiclist (indate, route, depdate, groupid, groupname , productname , agentnet, customnet, totalset, reserve, signup, cansales, getdeposit, passdeposit, depositprice, flight, inoutpoint,status, groupmemo) 
                    VALUES ( '$getDate' ,'$temp[0]','$temp[1]','$temp[2]','$temp[3]','$temp[4]','$temp[5]','$temp[6]','$temp[7]','$temp[8]','$temp[9]','$temp[10]','$temp[11]','$temp[12]','$temp[13]','$temp[14]','$temp[15]','$temp[16]','$temp[17]')";
        ...
        fclose($fp);//關閉檔案
        $getDate2= date("YmdHis");
        echo $getDate2."<br>";
    }

    $sql="DELETE FROM dynamiclist where route like'%總計%' ";
    $result=execute_sql($link,"tw1_yssales",$sql);
    $sql = "UPDATE keyword SET updatetime='$getDate2'where id='1' ";
    $result=execute_sql($link,"tw1_yssales",$sql);
