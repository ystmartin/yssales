...
<body>
    ...
    $name     = $_POST["sp_name"];
    if($name!=""){
        $sp_name="and productname like '%$name%'"; 
    }else{
    $sp_name='';
    }
    //echo $name.$sp_name;
    ...
    <?php
            if($_COOKIE["sp_month"]!=""){
                $sp_month=$_COOKIE["sp_month"];
                $datelock='已鎖定'.$sp_month.'月份';
                $unlock='解除鎖定月份';
                $sp_month="and month(depdate)='$sp_month'";
                
            }else{
                $datelock='';
                $unlock='';
            }
            //echo $_COOKIE["sp_month"].$datelock;
    
    echo '<div class="flex-container"></div>';
    echo '<div class="pc_head01_1 container-fluid">';
    echo '<div class="row sp_head">';
    echo '<div class="col-sm-1 item"><form name="form1" method="post" action="sprint.php"></div>';
    echo '<div class="col-sm-3 item">搜尋產品關鍵字：<input type="text" name="sp_name"><input type="submit" value="輸入"></div>';
    echo '<div class="col-sm-1 item"></form></div>';
    echo '<div class="col-sm-1 item"><form name="form2" method="post" action="sp_cookie.php"></div>';
    echo '<div class="col-sm-3 item">鎖定月份(如1或2...)：<input type="text" name="sp_month"><input type="submit" value="輸入">'.$datelock.'</div>';
    echo '<div class="col-sm-1 item"></form></div>';
    echo "<div class='col-sm-2 item'><a href='sp_cookie.php?sp_month="."clear"."'><font color='white'><h6>$unlock</h6></font></a></div>";
    echo "</div>";
    echo "</div >";
    echo "</div >";
    ...
    if ($route != "") {
        ...
        where route = '$route' and sp_flag = 'Y' ".$sp_name.$sp_month."order by depdate ASC";
        ...
        JOIN dynamiclist
        ON spfclist.`groupid` = dynamiclist.`groupid`
        where sp_flag = 'Y' ".$sp_name.$sp_month."order by depdate ASC";
        //     $result3 = execute_sql($link, "tw1_yssales", $sql3);
        $route_dsp = "全線";
    }
