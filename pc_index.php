...
date_default_timezone_set("Asia/Shanghai");
    ...
    <tbody>
        ...
        if($_COOKIE["sel_date"]!=""){
            $datelock='已鎖定'.$_COOKIE["sel_date"].'無其他篩選條件';
            $unlock='解除搜尋日期';
        ...
        <div class="pc_head02 container-fluid">
            ...
            <div class="pc_head02_1 row">
                ...
                echo "<div class='col-sm-1 item'><a href='pc_cookie.php?check05=" . $key05 . "'><font color=" . $chkcolor05 . "><h6>" . $check05 . "</h6></font></a></div>";
                echo "<div class='col-sm-1 item'><a href='pc_cookie.php?check04=" . $key04 . "'><font color=" . $chkcolor04 . "><h6>" . $check04 . "</h6></font></a></div>";
                echo '<div class="col-sm-1 item"><form name="form1" method="post" action="pc_cookie.php"></div>';
                echo '<div class="col-sm-3 item">查詢日期(如2024-01-01)：<input type="text" name="sel_date"><input type="submit" value="輸入">'.$datelock.'</div>';
                echo '<div class="col-sm-1 item"></form></div>';
                echo "<div class='col-sm-2 item'><a href='pc_cookie.php?sel_date="."clear"."'><font color='white'><h6>$unlock</h6></font></a></div>";
                //echo $_COOKIE["sel_date"];
                ?> 
            </div>
        </div>
        ...
        <table class="table table-striped">
            ...
            <tbody>
                <div class="row head03_txt">
                    ...
                    if ($key05 == "Y") {
                        $team05 = "and groupmemo like '%$check05%'";
                    } else {
                        $team05 = "";
                    }
                    ...
                    $sql3 = "SELECT * FROM dynamiclist" . $setroute . $setgroupname . $condtion . "productname not like '%TKT%' and productname not like '%特別團%'" . $team01 . $team02 . $team03 . $team05 .$team04 . "order by depdate ASC";
                    
                    if($search!=''){
                        $sql3 = "SELECT * FROM dynamiclist where productname like '%$search%'  or groupid like '%$search%' and ". "productname not like '%TKT%' and productname not like '%特別團%'" . $team01 . $team02 . $team03 . $team04 . "order by depdate ASC";

                    }

                    $sel_date=$_COOKIE["sel_date"];
                    //echo $_COOKIE["sel_date"];
                    if($sel_date!=''){
                        $sql3 = "SELECT * FROM dynamiclist where depdate='$sel_date' order by depdate ASC";
                    ...
                    if ($result3->num_rows > 0) {
                        ...
                        while ($row3 = $result3->fetch_assoc() and $j <= $records_per_page) {
                            ...
                            if ($row4['fc_flag'] == 'Y') {
                                ...
                                $fc_button = "<div class='pc_button-1'>有</div>";
                            } else {
                                $fc_button = "<div class='pc_button-2'>沒</div>";
                            };
                            echo "<td align='center' width='4%'>" . "$fc_button" . "</td>";
        ...
        for( $i=1 ; $i<=$total_pages ; $i++ ) {
            if ( $page-1 < $i && $i < $page+1 ) {
                ...
                $nextpage=$i+2;
                echo "<a href=pc_index.php?page=".$nextpage.">"."下二頁"."</a> ";
                $nextpage=$i+3;
                echo "<a href=pc_index.php?page=".$nextpage.">"."下三頁"."</a> ";
            }
        }
        ...
        //echo '<br>';
        $link = create_connection();
        $sql = "SELECT * FROM keyword where id='1'";
        $result = execute_sql($link, "tw1_yssales", $sql);
        $row = mysqli_fetch_assoc($result);
        echo "<p align='center'><h7>".$row['updatetime'].'程式版本:Rev.2.0.1,程式更新日期2024/1/25'."</h7></p>";
